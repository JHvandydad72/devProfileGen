const getBtn = document.getElementById('get-btn');
 ...
 const getData = () => {
     ...
     axios.get(`https://api.github.com/users/${userNameInput}`).then(response => {
         ...
         userName = `Hello! I'm ${response.data.login}!`
         userLocation = response.data.location
         userProfileLink = `${response.data.html_url}`;
         userBlog = `${response.data.blog}`
         userBio = `About me :${response.data.bio}`
         numberOfPR = `# of Repos :${response.data.public_repos}`
         numberOfFollowers = `# of Followers :${ response.data.followers}`
         numberOfFollowing = `# of Following :${response.data.following}`
